...
/*
THE DEFINITION OF FIELDS PROCESSING START
*/
...
class main_fields{
    public:
        ...
        int displaying_row_value;
        unsigned int start_position;
        unsigned int current_first_row_position;

    std::vector<std::string> define_displaying_contents(std::vector<std::string> file_contents){
        /*
        The number of top of row is 0
        */
        std::vector<std::string> return_file;

        conents = openFile(file_path);
        for(int i=0;i<displaying_row_value;i++){
            return_file[0] = file_contents[i+start_position];
        }
        current_first_row_position = start_position;
        return return_file;
    }

    std::vector<std::string> up_displaying_contents(void){
        std::vector<std::string> return_file;
